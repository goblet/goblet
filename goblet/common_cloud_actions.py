...
def create_cloudbuild(client, req_body):
    ...
    try:
        resp = client.execute(
            ...
            params={"body": req_body, "parent": f"projects/{defaultProject}/locations/{defaultLocation}"}
        )
        log.info(f"creating cloudbuild")
    except HttpError as e:
        raise e
    client.wait_for_operation(resp["name"], calls="operations")
...
def deploy_cloudrun(client, req_body, name):
    ...
    try:
        resp = client.execute(
            "create", 
            parent_key="parent",
            parent_schema="projects/" + get_default_project_number() + "/locations/{location_id}",
            params={"body": req_body, "serviceId": name}
        )
        log.info(f"creating cloudrun")
    except HttpError as e:
        if e.resp.status == 409:
            log.info(f"updating cloudrun")
            resp = client.execute(
                "patch",
                parent_key="name",
                parent_schema="projects/" + get_default_project_number() + "/locations/{location_id}/services/" + name,
                params={"body": req_body},
            )
        else:
            raise e
