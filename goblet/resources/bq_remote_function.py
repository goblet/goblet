...
class BigQueryRemoteFunction(Handler):
        ...
        try:
            ...
            if e.resp.status == 409:
                ...
                bq_connection = client.execute(
                    "get", params={"name": client.parent+connection_id}, parent=False, parent_schema="projects/{project_id}/locations/{location_id}/connections/"
                )
                pass
            else:
                raise e
    ...
    def _destroy_bigquery_connection(self):
        connection_id = f"{self.name}"
        client = self.versioned_clients.bigquery_connections
        result = client.execute("delete", params={"name": client.parent + connection_id}, parent=True)
        print(result)
